import type { Icon } from "@/types";

const SearchIcon = (props: Icon) => {
  const { width = 40, height = 40, color = "currentColor" } = props;
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M26.2584 23.7584H24.9417L24.475 23.3084C26.1084 21.4084 27.0917 18.9417 27.0917 16.2584C27.0917 10.275 22.2417 5.42505 16.2584 5.42505C10.275 5.42505 5.42505 10.275 5.42505 16.2584C5.42505 22.2417 10.275 27.0917 16.2584 27.0917C18.9417 27.0917 21.4084 26.1084 23.3084 24.475L23.7584 24.9417V26.2584L32.0917 34.5751L34.5751 32.0917L26.2584 23.7584ZM16.2584 23.7584C12.1084 23.7584 8.75838 20.4084 8.75838 16.2584C8.75838 12.1084 12.1084 8.75838 16.2584 8.75838C20.4084 8.75838 23.7584 12.1084 23.7584 16.2584C23.7584 20.4084 20.4084 23.7584 16.2584 23.7584Z"
        fill={color}
      />
    </svg>
  );
};
export default SearchIcon;
